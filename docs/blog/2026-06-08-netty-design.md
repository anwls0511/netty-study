...
> 공부용 `netty-study` 프로젝트에서 Netty TCP 서버를 작게 구현하면서 정리한 글입니다.

## 목차

- [1. Netty를 사용한 이유](#1-netty를-사용한-이유)
- [2. TCP는 메시지 경계가 없다는 점](#2-tcp는-메시지-경계가-없다는-점)
- [3. Decoder를 둔 이유](#3-decoder를-둔-이유)
- [4. ChannelPipeline 구조](#4-channelpipeline-구조)
- [5. Handler와 Service를 분리한 이유](#5-handler와-service를-분리한-이유)
- [6. 현재 프로젝트의 메시지 처리 흐름](#6-현재-프로젝트의-메시지-처리-흐름)
- [7. 개선할 점](#7-개선할-점)
- [8. 면접에서 설명할 수 있는 포인트](#8-면접에서-설명할-수-있는-포인트)

## 프로젝트 요약

| 항목 | 내용 |
| --- | --- |
| 목적 | Netty TCP 서버 구조 학습 |
| 메시지 형식 | 한 줄 JSON |
| 메시지 경계 | 줄바꿈(`\n`) |
| 주요 Decoder | `LineBasedFrameDecoder`, `StringDecoder`, `DeviceStatusJsonDecoder` |
| 주요 Handler | `DeviceStatusHandler` |
| 저장 방식 | `ConcurrentHashMap`에 최신 장비 상태 저장 |
...
| 단계 | 클래스 | 역할 |
| --- | --- | --- |
| 1 | `LineBasedFrameDecoder` | 줄바꿈 기준으로 하나의 메시지를 자른다. |
| 2 | `StringDecoder` | `ByteBuf`를 UTF-8 문자열로 변환한다. |
| 3 | `StringEncoder` | 서버가 `OK`, `ERROR` 같은 응답 문자열을 보낼 수 있게 한다. |
| 4 | `DeviceStatusJsonDecoder` | JSON 문자열을 `DeviceStatus` 객체로 변환한다. |
| 5 | `DeviceStatusHandler` | 변환된 객체를 받아 서비스 계층에 저장을 요청한다. |
...
| 개선 항목 | 설명 |
| --- | --- |
| 메시지 프로토콜 | 줄바꿈 기반 대신 길이 필드 기반 프로토콜을 검토할 수 있다. |
| 검증 로직 | `deviceId`, 온도, 습도, timestamp가 정상인지 확인해야 한다. |
| 저장소 | 현재는 메모리 저장이라 서버 재시작 시 데이터가 사라진다. |
| Decoder 테스트 | 메시지가 조각나서 들어와도 정상 복원되는지 테스트해야 한다. |
| 종료 처리 | 실제 애플리케이션에서는 graceful shutdown을 더 명확히 처리하는 편이 좋다. |

줄바꿈 기반 프로토콜은 이해하기 쉽지만, 메시지 본문에 줄바꿈이 들어갈 수 있거나 바이너리 데이터를 다뤄야 한다면 적합하지 않을 수 있다. 그런 경우에는 `LengthFieldBasedFrameDecoder`처럼 길이 필드 기반 프로토콜을 검토할 수 있다.

현재는 JSON이 `DeviceStatus`로 변환되면 바로 저장한다. 실제로는 `deviceId`가 비어 있는지, 온도나 습도 값이 말이 되는 범위인지, timestamp가 정상인지 확인해야 한다.
...
> TCP는 스트림 기반이라 메시지 경계를 보장하지 않습니다. 그래서 서버에서 Decoder를 두고 애플리케이션 메시지 단위로 복원해야 합니다.

### Pipeline 구조

> Netty는 들어온 데이터를 Pipeline에 등록된 Handler들이 순서대로 처리합니다. 이번 예제에서는 프레임 분리, 문자열 변환, JSON 변환, 서비스 호출을 각각 다른 단계로 나눴습니다.

### Handler와 Service 분리

> Handler는 네트워크 이벤트를 처리하고, Service는 비즈니스 로직을 처리하도록 나눴습니다. 이렇게 하면 테스트가 쉬워지고 저장 방식이 바뀌어도 네트워크 계층 변경을 줄일 수 있습니다.

### 현재 예제의 한계

> 지금은 줄바꿈 기반 JSON 메시지와 메모리 저장소를 사용한 공부용 예제입니다. 실제 서비스로 확장하려면 메시지 검증, 영속 저장소, 예외 처리, Decoder 테스트가 더 필요합니다.
